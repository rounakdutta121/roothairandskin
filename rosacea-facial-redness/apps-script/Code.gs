/**
 * Root Hair & Skin - Rosacea / Facial Redness landing trackers
 *
 * SEPARATE from the pigmentation landing Apps Script.
 *
 * SETUP
 ...
 * 4. Copy the Web App URL into rosacea-facial-redness/index.html as APPS_SCRIPT_URL
 *
...
var LANDING_LABEL = "Rosacea & Facial Redness";
...
function doGet() {
  return jsonResponse({
    ok: true,
    service: "Root Hair & Skin - Rosacea trackers",
...
function buildSubject(type, data) {
  if (type === "form") {
    ...
    return "[RHS Rosacea Lead] Form submission from " + fullName;
  }
  if (type === "call") {
    return "[RHS Rosacea Click] Call button - gclid present";
  }
  return "[RHS Rosacea Click] iMessage button - gclid present";
}
