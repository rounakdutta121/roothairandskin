/**
 * Root Hair & Skin - PhotoFractional landing trackers
 *
 * SEPARATE from the pigmentation and rosacea landing Apps Scripts.
 *
 * SETUP
 ...
 * 4. Copy the Web App URL into photofractional/index.html as APPS_SCRIPT_URL
 *
 * Receives JSON POST body:
 * {
 *   type: "form" | "call" | "imessage",
 *   landing: "photofractional",
 *   page: string,
 *   timestamp: string,
 *   lead?: { name, phone, email, concern, message },
 *   attribution?: { gclid, gbraid, wbraid, utm_source, ... }
 * }
 */

var NOTIFY_EMAIL = "damnart.seo@gmail.com";
var LANDING_LABEL = "PhotoFractional";
...
function doGet() {
  return jsonResponse({
    ok: true,
    service: "Root Hair & Skin - PhotoFractional trackers",
...
function buildSubject(type, data) {
  if (type === "form") {
    ...
    return "[RHS PhotoFractional Lead] Form submission from " + fullName;
  }
  if (type === "call") {
    return "[RHS PhotoFractional Click] Call button - gclid present";
  }
  return "[RHS PhotoFractional Click] iMessage button - gclid present";
}

function buildBody(type, data) {
  var lines = [];
  lines.push("Root Hair & Skin - " + LANDING_LABEL + " tracker");
  lines.push("=====================================");
  lines.push("Landing: " + LANDING_LABEL);
